...
+ sum_basic_product是个很重要的表，记录了我们一般都十分关心的用户行为，比如新增、活跃、使用时间等，里面的每个field基本都通过一条单独的sql来统计并插入，这里采用的是“insert on key duplicate update”的方式，之前对这种方式不太了解，网上找了段它和“replace”的对比
